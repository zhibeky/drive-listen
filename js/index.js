...
function toggleActiveClassOnLocation(locationId) {
  ...
  const activeLocationElement = document.querySelector(
    `.cities-list li.active`
  );

  if (activeLocationElement) activeLocationElement.classList.remove("active");
...
function updateCitiesList() {
  const data = LOCATIONS.map(
    ({ name, id }) =>
      `<li data-id="${id}" onclick="changeLocation(${id})">${name}</li>`
  ).join("\n");
